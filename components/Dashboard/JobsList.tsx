    ...
    <div className="p-4 sm:p-6">
      ...
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <div>
          ...
          <p className="text-slate-400 text-xs sm:text-sm mt-1">{filteredJobs.length} active jobs</p>
        </div>
        <button className="flex items-center gap-1.5 sm:gap-2 bg-cyan-600 hover:bg-cyan-500 text-white px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg transition-colors text-xs sm:text-sm font-medium">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <select
            ...
            className="flex-1 sm:flex-none bg-slate-800/50 border border-slate-700 rounded-lg py-2.5 px-3 sm:px-4 text-sm text-slate-200 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50 transition-all"
          >
          ...
          <button className="flex items-center gap-2 bg-slate-800/50 border border-slate-700 text-slate-300 px-3 sm:px-4 py-2.5 rounded-lg hover:bg-slate-700 transition-colors text-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
        {filteredJobs.map((job) => (
          ...
          >
            {/* Property Image */}
            <div className="relative h-28 sm:h-32 bg-slate-700 overflow-hidden">
              {job.property_image ? (
                <img
              ...
              <div className="absolute bottom-2 sm:bottom-3 left-2 sm:left-3 right-2 sm:right-3 flex items-center justify-between">
                <span className={`text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full border ${getStatusColor(job.status)}`}>
                  {formatStatus(job.status)}
                </span>
                <span className="text-[10px] sm:text-xs text-white/80 bg-slate-900/60 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full">
                  {job.progress}%
                </span>
              </div>
            </div>
            ...
            <div className="p-3 sm:p-4">
              <div className="flex items-start justify-between mb-2">
                <div className="min-w-0 flex-1 mr-2">
                  <h3 className="text-white font-semibold text-sm sm:text-base group-hover:text-cyan-400 transition-colors truncate">
                    {job.address}
                  </h3>
                  <p className="text-slate-400 text-xs sm:text-sm">{job.city}, {job.state}</p>
                </div>
                <button
                  onClick={(e) => { e.stopPropagation(); }}
                  className="p-1 rounded hover:bg-slate-700 text-slate-400 flex-shrink-0"
                >
                  <MoreHorizontal className="w-4 h-4" />
                </button>
              </div>

              <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-400 mb-2 sm:mb-3">
                <User className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                <span className="truncate">{job.owner_name}</span>
              </div>

              <div className="flex items-center justify-between pt-2 sm:pt-3 border-t border-slate-700/50">
                <div>
                  <p className="text-[10px] sm:text-xs text-slate-500">Estimate</p>
                  <p className="text-white font-semibold text-sm sm:text-base">{formatCurrency(job.estimated_total)}</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] sm:text-xs text-slate-500">Approved</p>
                  <p className="text-cyan-400 font-semibold text-sm sm:text-base">{formatCurrency(job.approved_amount)}</p>
                </div>
              </div>

              {/* Progress Bar */}
              <div className="mt-2 sm:mt-3">
                <div className="w-full h-1 sm:h-1.5 bg-slate-700 rounded-full overflow-hidden">
                  <div
