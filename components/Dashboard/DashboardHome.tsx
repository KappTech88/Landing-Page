      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 overflow-hidden">
          ...
          <div className="divide-y divide-slate-700/50">
            {recentJobs.map((job) => (
              <div key={job.id} className="p-4 hover:bg-slate-700/30 transition-colors cursor-pointer">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-white font-medium">{job.address}</h3>
                  <span className="text-cyan-400 font-semibold text-sm">{job.amount}</span>
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-500 text-xs">{job.progress}%</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 overflow-hidden">
          ...
          <div className="divide-y divide-slate-700/50">
            {upcomingTasks.map((task) => (
              <div key={task.id} className="p-4 hover:bg-slate-700/30 transition-colors cursor-pointer">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <h3 className="text-white font-medium mb-1">{task.task}</h3>
